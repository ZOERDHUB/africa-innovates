...
create table public.site_config (
  ...
  event_kicker text not null default 'ZOERD × ZCASH GHANA',
  ...
  event_date text not null default '[DATE TO BE CONFIRMED]',
  location text not null default '[LOCATION TO BE CONFIRMED]',
  duration text not null default '[DURATION TO BE CONFIRMED]',
  ...
  vote_price_zec text not null default '[TO BE CONFIRMED]',
...
insert into public.faq_items (question, answer, sort_order) values
 ...
 ('What happens if I forget the memo?','The team cannot automatically associate the payment with a participant. Contact the organisers with your transaction ID.',8),
 ('How is my vote verified?','The transaction is checked against the official voting wallet, amount, memo tag, confirmation status and voting day before the vote is counted.',9),
